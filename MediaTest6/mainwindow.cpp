...
void MainWindow::getVolume()
{
    if (m_mediaPlayer != nullptr)
    {
        ...
        if(mute == 1)
        {
             ui->pushButton_volume->setStyleSheet("border-image:url(:/Resources/media-png/mute1.png)");
        }else
        {
             ui->pushButton_volume->setStyleSheet("border-image:url(:/Resources/media-png/volume1.png)");
        }
        qDebug() << "mute:" << mute;
...
void MainWindow::on_pushButton_max_clicked()
{
    ...
    if(pWindow->isMaximized())
    {
        ...
        ui->pushButton_max->setToolTip("max");
    }
    else
    {
        pWindow->showMaximized();
        ui->verticalWidget->showMaximized();
        ui->pushButton_max->setToolTip("min");
    }
}
